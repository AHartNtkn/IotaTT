  ...
  case whnf c tr of
    ...
    Ok (AV n) ->
      case (g , n) of
        ...
        (Snoc g k , n) -> infer c g (AV (n - 1)) >>= \tr -> Ok (incFree tr)
    Ok (ALam (AAnn ty tr)) ->
    ...
    Ok (AAnn aty tr) ->
      case g of
        ...
        _ ->  Bad "Annotation apeared without being added to local context durring type inference."
    Ok (AApp tr1 tr2) ->
  ...
  case whnf c tr of
    ...
    Ok (AAnn aty tr) ->
      case g of
        ...
        _ -> Bad "Annotation apeared without being added to local context."
    Ok (AApp tr1 tr2) ->
    ...
    Ok (APi ty1 ty2) ->
      case nwhnf c ty of
        ...
        _ -> Bad "Pi types can only have AStar kind."
    Ok (AIPi ty1 ty2) ->
      case nwhnf c ty of
        Ok AStar -> check c g ty1 AStar >> check c (Snoc g ty1) ty2 AStar 
        _ -> Bad "Implicit products can only have AStar kind."
    Ok (AIota ty1 ty2) ->
      case nwhnf c ty of
        Ok AStar -> check c g ty1 AStar >> check c (Snoc g ty1) ty2 AStar 
        _ -> Bad "Dependent intersections can only have AStar kind."
    Ok (AId x y) ->
      case nwhnf c ty of
        Ok AStar -> Ok () -- Perhapse a better check is needed. x and y *should* be valid terms.
        _ -> Bad "Heterogenious equalities can only have AStar kind."
    Ok AStar ->
      case nwhnf c ty of
        Ok AStar -> Ok ()
